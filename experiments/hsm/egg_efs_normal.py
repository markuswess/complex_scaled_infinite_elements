...
if __name__=='__main__':
    ...
    for shift,scalepoint in zip(shifts,scalepoints):
        evs,gf,fes,m = egg_fqsc_normal(shift)
        plot(evs.real,evs.imag,'x')
        plot(evs[0].real,evs[0].imag,'o')
        approx_evs.append(evs[0])
        Draw(gf.components[0].components[0]/complex(gf.components[0].components[0](m(*scalepoint))),m,'asf')
        Redraw(blocking = True)
        #input('save')
        #SnapShot('../../'+'images/egg_res_normal_{:.3}_{:.3}'.format(evs[0].real,evs[0].imag).replace('.','_')+'.png')
        #input('calc next')
    print(approx_evs)
    show()
